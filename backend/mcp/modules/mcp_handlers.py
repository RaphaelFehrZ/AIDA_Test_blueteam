...
async def _handle_load_assessment(arguments: dict, mcp_service) -> List[TextContent]:
    ...
    if recon_data:
        response += "\n## Reconnaissance Data\n"
        # Group by data_type dynamically
        from collections import Counter
        type_counts = Counter(r.get('data_type', 'unknown') for r in recon_data)
        for dtype, count in type_counts.most_common():
            items = [r.get('name', '') for r in recon_data if r.get('data_type') == dtype]
            preview = ', '.join(items[:5])
            if count > 5:
                preview += f", ... (+{count - 5} more)"
            response += f"**{dtype}** ({count}): {preview}\n"
        response += "\n*Use `list_recon` for full details.*\n"
